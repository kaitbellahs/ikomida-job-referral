...
class ReferralJob {
  ...
  async run() {
    try {
      this.logger.info(`Referrals calculator started...!`)
      ...
      for (const referral of referrals) {
        ...
        for (const contract of contracts) {
          const payments = await contract?.$get('contractPayments', {
            order: [['confirmedDate', 'ASC']],
            where: {
              status: {
                [Domain.SqlDB.Op.in]: [
                  Types.Types.TAsaasPaymentStatus.CONFIRMED,
                  Types.Types.TAsaasPaymentStatus.RECEIVED
                ]
              },
              confirmedDate: {
        ...
        for (let index = 0; index < (bonusLevels?.length ?? 0); index++) {
          if ((usersByReferral?.length ?? 0) > 0) {
            ...
            for (const userByReferral of usersByReferral) {
              ...
              const userReferralRevenues: DBModels.ReferralRevuneModel[] | undefined = await userReferral?.$get(
                'referralRevunes',
                {
                  order: [['createdAt', 'ASC']],
                  limit: 1,
            ...
            bonusDetails.push({
              level: index,
              percentage: bonusLevels?.[index],
              resellers: usersByReferral?.length,
              total: levelTotal,
              bonus: levelBonus
            })
